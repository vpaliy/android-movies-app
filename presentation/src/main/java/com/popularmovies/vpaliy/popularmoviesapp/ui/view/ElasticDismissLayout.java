    ...
    public void onStopNestedScroll(View child) {
        if (Math.abs(totalDrag) >= dragDismissDistance) {
            ...
            animate()
                    ...
                    .scaleY(1f)
                    .setDuration(200L)
                    .setInterpolator(PresentationUtils.getFastOutSlowInInterpolator(getContext()))
                    .setListener(null)
                    .start();
            totalDrag = 0;
    ...
    public static class SystemChromeFader extends ElasticDragDismissCallback {
        ...
        public void onDrag(float elasticOffset, float elasticOffsetPixels,
            ...
            if (elasticOffsetPixels > 0) {
                ...
                activity.getWindow().setStatusBarColor(PresentationUtils.modifyAlpha(activity.getWindow()
                        .getStatusBarColor(), (int) ((1f - rawOffset) * statusBarAlpha)));
            } else if (elasticOffsetPixels == 0) {
                // reset
                activity.getWindow().setStatusBarColor(PresentationUtils.modifyAlpha(
                        activity.getWindow().getStatusBarColor(), statusBarAlpha));
                activity.getWindow().setNavigationBarColor(PresentationUtils.modifyAlpha(
                        activity.getWindow().getNavigationBarColor(), navBarAlpha));
            } else if (fadeNavBar) {
                // dragging upward, fade the navigation bar in proportion
                activity.getWindow().setNavigationBarColor(
                        PresentationUtils.modifyAlpha(activity.getWindow().getNavigationBarColor(),
